...
def analyze_image(image_path: str, lat: float, lng: float) -> AnalysisResult:
    """Analyzes a civic issue image using CLIP zero-shot classification,
    edge-density severity estimation, and image feature embedding.

    Guaranteed to NEVER raise an exception under any circumstance.
    """
    try:
        # Check backend configuration toggle if available
        try:
            from app.config import settings
            if hasattr(settings, "ai_enabled") and not settings.ai_enabled:
                return _fallback()
        except Exception:
            pass
        ...
        with Image.open(image_path) as img:
            ...
            image_embedding = embeddings.encode_image(img_rgb)
            category, confidence = classifier.classify(image_embedding)
            sev = severity_module.estimate_severity(img_rgb, category, confidence)
